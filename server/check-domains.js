...
const checkDomains = async () => {
  try {
    console.log('Checking Vercel deployment domains...');
    ...
    // Check the deployment domains
    try {
      const { stdout } = await execAsync('vercel domains ls');
      console.log('Deployment domains:', stdout);
    } catch (error) {
      console.error('Error checking deployment domains:', error);
    }
  } catch (error) {
    console.error('Error checking Vercel deployment domains:', error);
  }
};
